...
The {Doku::Puzzle} class is an abstract class for working with Sudoku-like puzzles.  It implements the concepts of groups, glyphs and squares.  It provides the {Doku::Puzzle#[] []} {Doku::Puzzle#[]= []=} for reading and modifying instances of the puzzle.  It provides equality comparison and solution checking among other things.

The {Doku::SquareOnGrid} module provides functions used for Sudoku-like puzzles that can be drawn on a 2D grid.  It implements the string representations of the puzzle and an <code>initialize</code> method that creates puzzles from strings and a <code>to_s</code> methods that creates strings from puzzles.  It provides <code>get</code> and <code>set</code> methods for modifying the puzzle by its coordinates.

used to define classes the represent Sudoku-like puzzles.  The Puzzle class includes the {Doku::SolvableWithDancingLinks} module which provides methods for solving the puzzles using the Dancing Links algorithm.
